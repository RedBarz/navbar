...
const NavbarDesktop = () => {
  ...
  const [flexDirection, setFlexDirection] = useState("items-center");
  ...
  const toggleAdditionalSpaceWidth = () => {
    ...
    setFlexDirection((prevDirection) =>
      prevDirection === "items-center" ? "items-start" : "items-center"
    );
    setMargin((prevMargin) => (prevMargin === "ml-0" ? "ml-8" : "ml-0"));
    setHidden((prevHidden) => (prevHidden === "hidden" ? "" : "hidden"));
    setFlexCol((prevFlexCol) =>
      prevFlexCol === "flex-col" ? "flex-row" : "flex-col"
    );
  };
  ...
  return (
    <nav
      ...
      className="w-[104px] h-screen rounded-[28px] border border-transparent ml-10 border-gradient"
    >
      <div className=" w-[100%] h-[100%] rounded-[28px] navbar-color navbar-blur-1 flex flex-col">
        <div
          className={`flex flex-col justify-center ${flexDirection} border-b border-b-[#cc8b8b21] pb-[15px]`}
        >
          <div
            className={`w-[52px] h-[12px] flex flex-row justify-between ${margin} items-center mt-6 `}
          >
          ...
          <div className="mt-[30px] flex flex-row justify-center w-full">
            ...
            <div className={`ml-3 ${hidden}`}>
              <p className="uppercase text-white/40 text-[11px]">
                Product designer
              </p>
              <p className="text-white">Shreya Babulkar</p>
            </div>
          </div>
          <button
            onClick={toggleAdditionalSpaceWidth}
            className="border w-6 h-6 flex justify-center items-center border-[#662525] rounded-full text-[#B86E9F] absolute -right-[10px] top-[77px] bg-[#0f090cd3]"
          >
        ...
        >
          <div className={`flex flex-col items-center mt-4`}>
            <p className="uppercase text-white/50 text-[11px] my-5">main</p>
            <div className="flex flex-row items-center mb-5">
              <button
                onClick={toggleAdditionalSpace}
                className=" w-[56px] h-[56px] flex justify-center items-center rounded-xl border border-[#6625254e] bg-[#cc8b8b33] hover:bg-[#cc8b8b97] hover:duration-200 active:duration-200 active:bg-[#b86e9f8c] text-white backdrop-blur-xl hover:backdrop-blur-xl active:backdrop-blur-xl"
              >
                <GrAppsRounded size={24} />
              </button>
              <p className={`${hidden} text-white ml-5`}>Dashboard</p>
            </div>
            <div className={`${hidden}`}>
              {showAdditionalCode && (
                <div
                  className={`${hidden} h-[124px] w-[158px] border-gradient rounded-2xl border border-transparent relative -right-[100px]`}
                >
          ...
          <div className="flex flex-row justify-between items-center h-[150px]">
            <div>
              <LuWallet size={24} />
              <LuNewspaper size={24} />
              <SlBell size={24} />
            ...
            <div
              className={`${hidden} text-white/50 hover:text-white hover:duration-150 hover:cursor-pointer flex flex-col justify-start items-star`}
            >
              <p>Contracts</p>
              <p>Payment</p>
              <p>Notification</p>
        ...
        <div className="flex justify-between flex-col items-center h-full mb-10">
          <div className="flex flex-col items-center border-t border-t-[#cc8b8b21] pt-5">
            ...
            <p className="text-white/40 text-[11px] uppercase">team members</p>
